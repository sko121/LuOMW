...
import com.Lu.omw.bean.DicBean;
import com.Lu.omw.bean.GiftBean;
import com.Lu.omw.db.DictionaryDBHelper;
...
public class DicDBUtil {
	...
	public static void insert(Context context){
		//装载数据
		List<DicBean> dic = FileUtils.readDataFromAssets(context, 
				DicBean.class, "dictionary.txt");
		
		Log.d("lulu", dic.toString());
		//A helper class to manage database creation and version management.
		DictionaryDBHelper myDBHelper = new DictionaryDBHelper(context);
		//Create and/or open a database that will be used for reading and writing.
		SQLiteDatabase db = myDBHelper.getWritableDatabase();
		for(int i = 0; i < dic.size(); i++){
			DicBean dicBean = dic.get(i);
			ContentValues contentValues = new ContentValues();  //HashMap
			contentValues.put("en_name", dicBean.getEn_name());
			contentValues.put("cn_name", dicBean.getCn_name());
	...
	public static void insertBySql(Context context){
		...
		List<GiftBean> gifts = FileUtils.readDataFromAssets(context, 
				GiftBean.class, "dictionary.txt");
		MyDBHelper myDBHelper = new MyDBHelper(context);
		SQLiteDatabase db = myDBHelper.getWritableDatabase();
		long start = SystemClock.currentThreadTimeMillis();
		for(int i = 0; i < gifts.size(); i++){
			GiftBean gift = gifts.get(i);
			db.execSQL("insert into dic(name) values(?)", 
